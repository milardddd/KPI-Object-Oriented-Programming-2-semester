...
{
    public static readonly object CountPossibleToProlongDepositTestCases = new object[]
    {
    ...
    private const string AssemblyName = "Interfaces";
    ...
    public void ClientTotalIncomeReturnsSumOfIncomeOfAllDeposits(string className, decimal amount, int period, decimal expectedIncome)
    {
        var clientType = GetCustomType("Client", "Class 'Client'");
        var deposit = GetCustomType(className, $"Class '{className}'");
    ...
    [TestCase("BaseDeposit", 1000, 1, "LongDeposit", 1000, 7, "SpecialDeposit", 1000, 1, 150)]
    public void ClientMaxIncomeReturnsMaxIncomeOfAllDeposits(
        string className1,
        decimal amount1,
        int period1,
        string className2,
    ...
    [TestCase("BaseDeposit", 1000, 1, "LongDeposit", 1000, 7, "SpecialDeposit", 1000, 1, 150, 2)]
    public void ClientGetIncomeByNumberReturnsIncomeOfDepositWithIndexPlusOne(
        string className1,
        decimal amount1,
        int period1,
        string className2,
    ...
    /// <summary>
    /// WARNING: The area below is allowed only for interfaces tests.
    /// </summary>
    ...
    public void ClassImplementsLibraryGenericInterface(string className, Type interfaceType, string genericClassName)
    {
    ...
    public void TypeHasMethod(string typeName, string methodName, Type returnType, params Type[] parameters)
    {
        ...
        {
            Assert.Fail($"Type '{typeName}' doesn't have method with name '{methodName}' that returns {returnType}.");
        }
    }
    ...
    public void DepositCompareToComparesByGeneralSum(string className1, string className2, decimal amount1, int period1, decimal amount2, int period2, int expected)
    {
    ...
    [TestCaseSource(nameof(CountPossibleToProlongDepositTestCases))]
    ...
    {
        ArgumentNullException.ThrowIfNull(amountsForSpecialDeposits);
        ArgumentNullException.ThrowIfNull(periodsForLongDeposits);
        var clientType = GetCustomType("Client", $"Class 'Client'");
        ...
        for (int i = 0; i < 3; i++)
        {
            addDeposit.Invoke(client, new object[] { Activator.CreateInstance(baseDepositType, 1000m, 1) });
        }

        for (int i = 0; i < amountsForSpecialDeposits.Length; i++)
        {
            addDeposit.Invoke(client, new object[] { Activator.CreateInstance(specialDepositType, amountsForSpecialDeposits[i], 1) });
        }

        for (int i = 0; i < periodsForLongDeposits.Length; i++)
        {
            addDeposit.Invoke(client, new object[] { Activator.CreateInstance(longDepositType, 1000m, periodsForLongDeposits[i]) });
        }
    ...
    {
        var type = Type.GetType($"Interfaces.{className}" + ", Interfaces");
